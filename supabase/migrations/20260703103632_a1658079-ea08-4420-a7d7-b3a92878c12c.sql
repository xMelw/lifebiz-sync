...
CREATE OR REPLACE FUNCTION private.generate_order_number(_workspace_id uuid)
...
CREATE OR REPLACE FUNCTION private.tg_set_order_number()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF NEW.order_number IS NULL THEN
    NEW.order_number := private.generate_order_number(NEW.workspace_id);
  END IF;
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER orders_set_number
  ...
  FOR EACH ROW EXECUTE FUNCTION private.tg_set_order_number();

DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN SELECT id, workspace_id FROM public.orders WHERE order_number IS NULL ORDER BY created_at LOOP
    UPDATE public.orders SET order_number = private.generate_order_number(r.workspace_id) WHERE id = r.id;
  END LOOP;
END;
$$;

-- Tabela de ações do cliente
...
CREATE POLICY "members_select_client_actions" ON public.order_client_actions
  ...
  USING (EXISTS (
    ...
    WHERE o.id = order_id
    AND private.has_workspace_access(auth.uid(), o.workspace_id, 'negocio')
  ));

-- RPC: gerar link público e PIN
...
BEGIN
  ...
  IF NOT private.can_manage(auth.uid(), o.workspace_id, 'negocio') THEN
  ...
  WHERE id = _order_id;
  result := json_build_object('token', new_token, 'pin', new_pin, 'expires_at', (now() + interval '7 days')::TEXT);
  RETURN result;
END;
$$;
GRANT EXECUTE ON FUNCTION public.generate_order_public_link(uuid) TO authenticated;

-- RPC: verificar PIN (acesso anónimo)
...
BEGIN
  ...
  RETURN json_build_object(
    ...
    'total', o.total, 'signal_amount', o.signal_amount, 'event_date', o.event_date,
    'location', o.location, 'client_notes', o.client_notes, 'delivery_date', o.delivery_date,
    'workspace_name', o.workspace_name, 'expires_at', o.public_token_expires_at
  );
END;
$$;
GRANT EXECUTE ON FUNCTION public.verify_order_pin(text, text) TO anon, authenticated;

-- RPC: submeter ação do cliente (acesso anónimo)
...
-- Atualizar funções de stock
CREATE OR REPLACE FUNCTION private.order_reserves_stock(_status public.order_status)
...
BEGIN
  SELECT * INTO o FROM public.orders WHERE id = _order_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Encomenda não encontrada'; END IF;
  IF NOT private.can_write(auth.uid(), o.workspace_id, 'negocio') THEN RAISE EXCEPTION 'Sem permissão'; END IF;
